...
class PerturbPix:
    ...
    def _join_sub_mat(self, submats):
        ...
        for i in range(b):
            ...
            for j in range(a - 1):
                sub_m = np.hstack((sub_m,
                                   np.matrix(submats[s + j + 1].copy())))
            s += a
            sub_m = sub_m.flatten()
    ...
    def _rotate(self):
        ...
        for i in range(len(self.px_as_submats)):
            y = rotate(self.px_as_submats[i], 60 * i, resize=False)
    ...
    def _permute(self):
        perm_mats = []
        for i in range(len(self.px_as_submats)):
            #print self.px_as_submats[i]
            x = np.asarray(self.px_as_submats[i]).reshape(-1)
            #print x
            #print self.rot_mats[i]
            y = x[self.rot_mats[i]]
            perm_mats.append(y)
            #print y
        return perm_mats
    ...
    def recover(self, rot_type='orthonormal'):
        ...
        if rot_type == 'orthonormal':
            ...
            for i in range(len(self.rotated_mats)):
                z = np.round(
                    np.matmul(rotated_mats[i],
                              np.linalg.inv(self.rot_mats[i])), 2)
                rotated_twice.append(z)
...
def main():
    ...
    for pixels in np_data:
        print pixels
        rotated_pixel = PerturbPix(pixels, rot_mats, image_size, block_size,
                                   'rot+pert', 100.0)
        ...
        print perm_mats
        permuted_pixels = PerturbPix(rotated_pixel.pert_pixels, perm_mats,
                                     image_size, block_size, 'permute')
