...
class PerturbData:
    ...
    def _transform(self, np_data):
        ...
        for pixels in np_data.as_np_arrays:
            ...
            rotated_pixel = PerturbPix(pixels_in, self.rot_keys,
                                       self.t_p.image_size,
                                       self.t_p.block_size, self.shuffle_arr,
                                       self.action, self.t_p.noise_level)
    ...
    def _write2file(self, prtb_data, y=[], header_lbl="pixel"):
        ...
        if len(y) == len(prtb_data):
            ...
            data_type = "train"
        data.to_csv(
            'mnist_%s_%s_%s_%s.csv' %
            (data_type, self.t_p.noise_level, self.t_p.block_size[0],
             self.t_p.block_size[1]),
            sep=',',
            index=False)
...
def main():
    ...
    rot_keys = PerturbMat(trans_prop.image_size, trans_prop.block_size,
                          trans_prop.rot_type).Rot_mat
